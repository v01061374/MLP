...
@endsection
@section('content')
    <script>window.IntercomSetting = {"app_id": 'aywrlu55'}</script>
            ...
            <div class="s-r-c">
                <div class="wrapper full-style">
                    ...
                    <div class="left-side-holder si-h visible-md visible-lg" id="si-h">
                        <div class="side-section" id="sidebar-region">
                            <div>

                                <div class="white-box">
                                    <div class="qu-s">
                                        <h5><i class="fo fo-restaurant"></i> جستجوی سریع فروشگاه ها</h5>
                                        <div class="si-cnt clearfix" id="qu-s-list">
                                            <ul class="thin-scrollbar clearfix">
                                                <li class="{{(isset($activeCategory['id'])) ?  "" : "active"}}">
                                                    @if($zone)
                                                        <a href="{{route('stores.index', ['zone' => array_search($zone , $zones)])}}">
                                                ...
                                                @foreach($categories as $cat)
                                                    <li class="{{(isset($activeCategory['id']) && $activeCategory['id'] == $cat['id']) ? "active" : ""}}">
                                                        @if($zone)
                                                            <a href="{{route('stores.index', ['category' => $cat['id'], 'zone' => array_search($zone , $zones)])}}"><span>{{$cat['title']}}</span></a>
                                                        @else
                                                            <a href="{{route('stores.index', ['category' => $cat['id']])}}"><span>{{$cat['title']}}</span></a>
                                                        @endif
                    ...
                    <div class="right-side-holder">
                        <div class="s-r-b white-box clearfix">
                            <header class="">
                                <h3 id="search-header">فروشگاه ها در "<b>{{$zone?? 'همه محله های تهران'}}"</b> شامل
                                    "<b>{{$activeCategory['title']??'همه دسته بندی ها'}}</b>"
                                </h3>
                                <small id="total-result">{{count($stores)}} فروشگاه</small>
                            </header>

                            <div class="r-l clearfix">
                                <div>
                                    @foreach($stores as $store)
                                        @if($store['isVerified'])
                                            <a class="r-i" href="/store/{{$store['id']}}">
                                                <section>
                                                    <figure class="re-c " style="background-color: #7f4113">

                                                        <div class="re-c-img fade-me lazyloaded"
                                                             style="background-image: url({{asset('/storage/' . $store['coverImage'])}})"></div>


                                                        <!--<span class="free-delivery"></span>-->

                                                        @if(isset($storeCategoryProducts[$store['id']]))
                                                            <div class="pic-list">
                                                                <ul>
                                                                    @foreach($storeCategoryProducts[$store['id']] as $product)

                                                                        @if($product['photo'])

                                                                            <li title="{{$product['title']}}"
                                                                                style="background: url({{asset('/storage/' . $product['photo'])}}); background-position: center; background-size: cover">
                                                                            </li>
                                                                        @else
                                                                            <li title="{{$product['title']}}">
                                                                            </li>
                                                                ...
                                                                <b>{{count($storeCategoryProducts[$store['id']])}}
                                                                    مورد</b>
                                                            </div>
                                                        @endif
                                                    ...
                                                    <div class="re-d">
                                                        ...
                                                        <aside>
                                                            <h4>{{$store['title']}}</h4>
                                                            <div class="mini-info">
                                                                <div><i class="fo fo-pin"></i> {{$store['address']}}
                                                                </div>
                                                            </div>
...
@section('scripts')
    <script>
        $(document).ready(function () {
            var selectedZone = $('#zone-select option[value="{{array_search($zone, $zones)}}"]');
            selectedZone.prop('selected', true);
            var selectedCategory = $('#category-select option[value="{{$activeCategory['id']?? "-1"}}"]');
            selectedCategory.prop('selected', true);
            $('#zone-select').select2();
            ...
            $('.search-form .btn-search').click(function () {
                ...
                url = url + "?category=" + category + "&zone=" + zone;
                window.location = url;
